...
Done in the style of the micropython apps with an install.py reading a toml configuration that generates a cfg.py.
it does no IO when running just waiting on a mqtt subscribe(s) and sending a pre defined email with jpg images.
See the example.toml file for examples
