...
my $XbeeSendQueue_id = 'XbeeSendQueue';
...
sub enqueue {
    ...
    if (ref($din) eq "HASH")
    {
       ...
       #my $item = Data::Dumper->Dump([$din],["msg"]);
       my $item = Storable::freeze($din);
       my $lth = length($item);
       if ($lth > $max_size)
       {
        ...
        #{
            #$self->{mq}->blocking(1);
        #}
       my $status = $self->{mq}->send("H".$item);
       if (!$status)
       {
           DBG&&$fp->prt("enqueue cannot send: H $!");
       }
       #printf "[%s][%s]\n", length($item), $buffer;
    }
    else
    {
       $din='X' if (!defined $din);
       my $lth = length($din);
       if ($lth > $max_size)
       {
            DBG&&$fp->prt("QueueManager::enqueue message size exceeded [%s] this message = %s  max = %s", $self->{name}, $lth, $max_size);
            $self->{dt}->do("UPDATE queue_size SET largest_msg = %s WHERE  queue = %s", $lth, $self->{name});
            ## confess "QueueManager::enqueue message size exceeded";
       }
       elsif (!$self->{mq}->send("S".$din)) #, $self->{wait}))
       {
           DBG&&$fp->prt("enqueue cannot send: S $!");
       }
       ### $self->{mq}->send("S".$din,0) or die "enqueue cannot send: S $pname $!\n";
    }
...
sub dequeue {
    ...
    my $in_queue = $self->{mq}->attr()->{mq_curmsgs};
    DBG&&$fp->prt("[%s][%s] timeout = [%s] [%s] queue count[%s]",  $self->{name},  $self->{oflag}, $timeout||'none', GetNameByPID($self->{dt}, $in_queue));
    my ($msg,  $prio);
    if ($timeout)
    {
    ...
    #my $msg; eval $data;
    #undef $data;
    my %ret = %{Storable::thaw(substr($msg,1))};
    #print Dumper %ret;
    return \%ret;
}
...
sub destroy_message_queues
{
    ...
    QueueManager::unlink_queue($XbeeSendQueue_id);
...
sub create_message_queues
{
    ...
    my $XbeeSendQueue   = QueueManager->new({name =>$XbeeSendQueue_id, create => 1, message_size => 255, queue_size => 300});
    ...
    return ($WorkerBeeQueue, $XbeeSendQueue, $TraceQueue, $EmailQueue, $Watchdog, $ProcessMsgQueue, $PacketQueue, $EvaluateQueue)
}
...
{
    my ($options) = @_;
    return  QueueManager->new({name =>$XbeeSendQueue_id, reader => $options->{reader}, nowait => $options->{nowait}});
}
sub TraceQueue
{
    my ($options) = @_;
