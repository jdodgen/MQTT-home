...
async def all_devices(request):
    ...
    if request.method == "POST":
        ...
        if parts[0] == "send":
            (topic, payload) = send_mqtt_publish(parts[2], parts[1])
            error=f"Command {topic} > {payload} published"
        elif parts[0] == "zbrefresh":
            publish.single(config.ZIGBEE2MQTT_BRIDGE_REQUEST_DEVICES,"")
            ...
            publish.single(mqtt_hello.hello_request_topic) 
