...
use constant FTP_SITE     => 'xxxxxx.com';
use constant FTP_USER     => 'goblidygoop';
use constant FTP_PASSWORD => 'password';
use constant FTP_PORT     => 22;

my constant $retry_delay_time = 10;
my constant $run_dir      = '/alertaway';
# $ENV{'PERL_LWP_SSL_VERIFY_HOSTNAME'} = 0;
system("dhclient");  # this is done incase the Ethernet connection was trashed
my $base_name = "aa";
if (-d 'test')
{
    $base_name = 'test';
...
while (1)
{
  ...
  $loop_count++;
  my $ftps = Net::SFTP::Foreign->new(FTP_SITE,
                        user => FTP_USER,
  ...
  # now we might have the two file names, local and remote
