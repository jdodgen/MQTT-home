...
CREATE TABLE alerts (
ah CHAR NOT NULL  REFERENCES wireless_devices (ah),
al CHAR(8) NOT NULL  REFERENCES wireless_devices (al),
port CHAR NOT NULL ,
last_range_state,
...
threshold_to,
last_date,
PRIMARY KEY (ah, al, port, contact)
);
...
CREATE INDEX actionsx1 ON actions (device_ah, device_al, device_port);
CREATE INDEX actionsx2 ON actions (ah, al, port);


drop table if exists alert_pictures;
CREATE TABLE alert_pictures (
ah CHAR NOT NULL  REFERENCES alerts (ah),
al CHAR(8) NOT NULL  REFERENCES alerts (al),
port CHAR NOT NULL  REFERENCES alerts (port),
camera_name CHAR DEFAULT NULL,
repeat_count INTEGER DEFAULT NULL,
repeat_delay INTEGER DEFAULT NULL,
PRIMARY KEY (ah, al, port, camera_name)
);
