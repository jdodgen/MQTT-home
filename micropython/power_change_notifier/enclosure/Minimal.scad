...
offset_z = (short_base == true) ? 0:12.4;  // 13
...
posts_total_height = (shell_z/2)-2.5; //usb_c_hole_height/2+shell_wall_thickness-2.5;
...
module make_s2_base() {
    difference()
    {
        ...
        z = (short_base == true) ? 
                    shell_z/2-usb_c_hole_height/2 
                    : 
                    shell_z -usb_c_hole_height-2;
        translate([s2_x+0.5, (shell_y-usb_c_hole_width)/2, z])
            usb_c_hole();
        hole_d = 4;
        
        hz = (short_base == true) ? shell_z/2-0.4 : shell_z-usb_c_hole_height+2.0;
        translate([shell_x-shell_wall_thickness-(hole_d/2)-2,0, hz]){
            rotate([-90,0,0])cylinder(h=shell_y, d=hole_d, $fn=30);
        }
        translate([shell_x-shell_wall_thickness-(hole_d/2)-4, (shell_y-hole_d/2)-2.5, hz]) {
            cube([hole_d,hole_d,10]);
        }
        
        if (double_cut_out == true)
            translate([shell_x-double_cutout_x, shell_y-double_cutout_y-double_cutout_y_offset, 0])
