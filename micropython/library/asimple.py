...
#
print_tag = "simple"
do_prints = False # usually set to False in production
...
class MQTTClient:
    ...
    def connect(self, clean_session=True, timeout=None):
            ...
            #await self.socket_lock.release()
            return resp[2] & 1
    ...
    async def subscribe(self, topic, qos=0):
        ...
        #print(".subscribe after the assert")
        pkt = bytearray(b"\x82\0\0\0")
        self.pid += 1
        struct.pack_into("!BH", pkt, 1, 2 + 2 + len(topic) + 1, self.pid)
        # print(hex(len(pkt)), hexlify(pkt, ":"))
        print(".subscribe before first write")
        self.sock.write(pkt)
        print(".subscribe after first write")
        self._send_str(topic)
        self.sock.write(qos.to_bytes(1, "little"))
        while 1:
            #print(".subscribe wait_msg loop")
            op = await self.wait_msg()
            print(".subscribe op", op);
            if op == 0x90:
                resp = self.sock.read(4)
    ...
    async def wait_msg(self):
        ...
        while True:
            ...
            try:
                ...
                print(".wait_msg OSError", str(e), type(e))
                await asyncio.sleep(1)
                if e.errno == -113:  # EHOSTUNREACH
                    print(".wait_msg EHOSTUNREACH")
                    #await self.socket_lock.release()
                    raise
        ...
        if res == b"\xd0":  # PINGRESP
            ...
            assert sz == 0
            a#wait self.socket_lock.release()
            return None
        op = res[0]
        if op & 0xF0 != 0x30:
            #await self.socket_lock.release()
