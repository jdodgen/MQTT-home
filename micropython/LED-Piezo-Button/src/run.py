...
alert  = feature_alert.feature("hot_water_controler",publish=True)
...
def callback(topic_in, msg_in, retained):
    ...
    if (topic == alert.topic()):
        ...
        if (msg == alert.payload_on()):
            ...
            ah.turn_off()
    elif (topic == mqtt_hello.request_topic()):
        hello(client)
...
async def conn_han(client):
    ...
    await client.subscribe(mqtt_hello.request_ip_devices_hello())  
    await hello(client)
